...
# .go 文件行数 >300 拒绝提交——要求先用 line-limit skill 拆分。
...
if [ -n "$BAD" ]; then
  echo "✗ 提交被拒：以下 Go 文件超过 300 行，需先用 line-limit skill 拆分：" >&2
  echo "$BAD" >&2
  echo >&2
  echo "  1) 看函数/方法行数分布：  go run ./skills/line-limit/scripts/asttool funcsize <file>" >&2
  echo "  2) 按主题拆文件：          go run ./skills/line-limit/scripts/asttool analyze <file>" >&2
  echo "                             go run ./skills/line-limit/scripts/asttool split <src.go> <out.go>:Name1,Name2" >&2
