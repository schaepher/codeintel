...
func domainPrompt(factsPath string) string {
	...
	b.WriteString("2. 每个域：name（中文业务名，如「商品域」）、description（一句话职责）、packages（归属包**完整路径**列表——用文件中 packages[].path）、tables（归属表名列表——用文件中 tables[].name）\n")
	b.WriteString("3. 包与表只归属一个域；文件里没有的包路径、表名一律不要写\n")
