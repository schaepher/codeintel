...
| 20 | 大仓库构建把机器拖死（换页、卡住、被 OOM 杀） | `/usr/bin/time -v <cmd>` 看 Max RSS / minor page faults / involuntary ctx switches；`[index] ssa 步骤` 行看各阶段 heap_mb；`make bench BENCH_REPO=<repo>` 看进程内峰值 | 内存峰值在 go/packages 类型检查 + SSA 构建阶段（Q246 实测 2.2-2.5GB），emit 阶段在峰值之下。预防：小内存机器别与其他重进程并发跑；记录基线再改 |
