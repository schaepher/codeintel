...
### 遗留待办（2026-08-23 盘点，保留优先级）

- **P0（需用户操作）XORM Settlement Service 真实仓库验证**：长期
  遗留——工具在真实复杂仓库的效果验证（表识别/字段追溯/ER 图/
  wiki）。用户提供仓库路径后执行。
- **P2 wiki serve 离线条目**：ER 图/架构图 mermaid 走 CDN，无网络
  时图空白（单文件 html 同病——f7496af 内嵌→f205d97 回退 CDN 的
  教训）。serve 版可加内嵌回退（体积 ~3.3MB，可仅 serve 场景）。
- **P3 LSP 编辑器集成**（#215 长期未做部分）：VS Code 扩展已覆盖
